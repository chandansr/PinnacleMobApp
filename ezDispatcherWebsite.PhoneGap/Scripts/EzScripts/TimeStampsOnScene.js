...
function GetOnSceneTimeStamp() {
    ...
    if (Page == "TimeStampOnScene") {
        $.ajax({
            ...
            success: function (data) {
                ...
                if (result.length > 0) {
                    $.each(result, function (i1, v1) {
                        if (v1.MasterFieldID != "") {
                            DynamicFieldHtml = DynamicFieldHtml + "<div class='OnSceneDynamicField' MasterFieldId='" + v1.MasterFieldID + "'>"
                    ...
                    if (result[0].IsDelay == "1") {
                        $("#OnSceneDelayReason").show();
                    }
                    else {
                        $("#OnSceneDelayReason").hide();
                    }
            ...
            complete: function () {
                $("#OnSceneDynamicFileds").html(DynamicFieldHtml);
...
function SaveOnSceneTimeStamps() {
    new ezphonemessege().show({
        msg: 'Processing..Please wait..'
    });
    ...
    jsondata.Obj = [];
    $(".OnSceneDynamicField").each(function () {
        hasJson = 1;
        var $this = $(this);
        jsondata.Obj.push({
    ...
    if ($('#OnSceneDelayReason').is(':hidden')) {
        jsondata.IsDelay = "0";
    }
    else {
        jsondata.IsDelay = "1";
        jsondata.DelayReason = $('#txtDelayReasonOnScene').val();
    }
