...
function GetDispatchedTimeStamp() {
    ...
    if (Page == "TimeStampDispatched") {
        $.ajax({
            ...
            success: function (data) {
                ...
                if (result.length > 0) {
                    $.each(result, function (i1, v1) {
                        if (v1.MasterFieldID != "") {                            
                            DynamicFieldHtml = DynamicFieldHtml + "<div class='DispatchedDynamicField' MasterFieldId='" + v1.MasterFieldID + "'>"
            ...
            complete: function () {
                $("#DispatchedDynamicFileds").html(DynamicFieldHtml);
...
function SaveDispatchedTimeStamps() {
    new ezphonemessege().show({
        msg: 'Processing..Please wait..'
    });
    ...
    jsondata.Obj = [];
    $(".DispatchedDynamicField").each(function () {
        hasJson = 1;
        var $this = $(this);
        jsondata.Obj.push({
