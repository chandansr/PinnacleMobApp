...
function GetArrivedTimeStamp() {
    ...
    if (Page == "TimeStampArrived") {
        $.ajax({
            ...
            success: function (data) {
                ...
                if (result.length > 0) {
                    $.each(result, function (i1, v1) {
                        if (v1.MasterFieldID != "") {
                            DynamicFieldHtml = DynamicFieldHtml + "<div class='ArrivedDynamicField' MasterFieldId='" + v1.MasterFieldID + "'>"
                    ...
                    if (result[0].IsDelay == "1") {
                        $("#ArrivedDelayReason").show();
                    }
                    else {
                        $("#ArrivedDelayReason").hide();
                    }
            ...
            complete: function () {
                $("#ArrivedDynamicFileds").html(DynamicFieldHtml);
...
function SaveArrivedTimeStamps() {
    ...
    jsondata.Obj = [];
    $(".ArrivedDynamicField").each(function () {
        hasJson = 1;
        var $this = $(this);
        jsondata.Obj.push({
    ...
    if ($('#ArrivedDelayReason').is(':hidden')) {
        jsondata.IsDelay = "0";
    }
    else {
        jsondata.IsDelay = "1";
        jsondata.DelayReason = $('#txtDelayReasonArrived').val();
    }
